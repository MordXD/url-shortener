...
def client():
    ...
    with patch('app.api.get_redis_client') as mock_get_redis:
        ...
        with patch('app.api.ShortenerService') as mock_shortener_class:
            ...
            shortener_instance.get_original_url = AsyncMock(return_value="https://example.com")
            mock_shortener_class.return_value = shortener_instance
            
            # Также патчим лимитер чтобы тесты не были лимитированы
            with patch('app.services.limiter.RateLimiter.is_rate_limited', return_value=False):
...
def test_redirect_not_found(client):
    ...
    with patch('app.services.shortener.ShortenerService.get_original_url', return_value=None):
